...
export function ArchivePagination({
  ...
  return (
    <nav
      ...
      aria-label={`${testIdPrefix === "customer" ? "客户" : "车辆"}档案分页`}
    ...
    >
      <p className="text-xs text-ink-soft dark:text-slate-400">
        显示 <span className="font-semibold tabular-nums text-ink dark:text-slate-200">{start}–{end}</span>
        {" "}条，共 <span className="font-semibold tabular-nums text-ink dark:text-slate-200">{total}</span> 条
      </p>
      <div className="flex flex-wrap items-center gap-1">
        ...
        <span className="px-1 text-[11px] text-ink-faint">/ {pageCount} 页</span>
        <button
          type="button"
