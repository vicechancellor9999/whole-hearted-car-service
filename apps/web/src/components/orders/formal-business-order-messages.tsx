    ...
    <div className="mx-auto max-w-5xl">
      ...
      {canCollaborate ? (
        <form onSubmit={submit} className="mt-4 border-b border-line pb-4">
          <textarea aria-label="评论内容" value={body} maxLength={4000} onChange={(event) => setBody(event.target.value)} onPaste={pastePhotos} required placeholder="写下评论；可以粘贴照片或 @ 同事…" className="min-h-24 w-full resize-y rounded-lg border border-line bg-layer-2 p-3 text-sm text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/10" />
