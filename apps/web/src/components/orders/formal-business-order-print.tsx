...
import {
  ...
  type FormalCustomerCopySnapshot,
...
import { BusinessPendingQuoteNotice } from "./business-pending-quote-notice";
  ...
  return (
    <section className="formal-print-section mt-4">
      ...
      {(Object.keys(CATEGORY_LABELS) as Array<keyof typeof CATEGORY_LABELS>).map((kind) => {
        ...
        return <section key={kind} className="formal-print-section mt-2"><h3 className="bg-blue-50 px-2 py-1 text-[10px] font-black text-blue-900">{CATEGORY_LABELS[kind][0]}{bilingual ? ` / ${CATEGORY_LABELS[kind][1]}` : ""}</h3><div className="grid grid-cols-[minmax(0,1.6fr)_minmax(0,1.2fr)_55px_48px_85px_80px_90px] border-b border-slate-300 px-2 py-1 text-[9px] font-semibold text-slate-500"><span>项目名称</span><span>描述</span><span>单位</span><span>数量</span><span className="text-right">含税单价</span><span className="text-right">本项折扣</span><span className="text-right">含税小计</span></div>{items.map((item, index) => <div key={`${kind}-${index}`} className="formal-print-row grid grid-cols-[minmax(0,1.6fr)_minmax(0,1.2fr)_55px_48px_85px_80px_90px] items-start border-b border-slate-200 px-2 py-2"><span><strong className="block">{item.nameZh}</strong>{bilingual && item.nameEn ? <small className="block text-[9px] text-blue-700">{item.nameEn}</small> : null}</span><span>{item.descriptionZh || "—"}{bilingual && item.descriptionEn ? <small className="block text-[9px] text-slate-500">{item.descriptionEn}</small> : null}</span><span>{item.kind === "labor" ? "JOB" : item.unitLabelZh}{bilingual && item.kind !== "labor" && item.unitLabelEn ? <small className="block text-[9px] text-slate-500">{item.unitLabelEn}</small> : null}</span><span>{item.quantity}</span><span className="whitespace-nowrap text-right tabular-nums">{item.pendingQuote ? (bilingual ? "待报价 / Pending quote" : "待报价") : formatFormalMoney(item.unitPriceMinor)}</span><span className="whitespace-nowrap text-right tabular-nums text-rose-700">{item.pendingQuote ? "—" : `−${formatFormalMoney(item.itemDiscountMinor)}`}</span><strong className="whitespace-nowrap text-right tabular-nums">{item.pendingQuote ? "—" : formatFormalMoney(item.subtotalMinor)}</strong></div>)}</section>;
      })}
...
function ChargeTotals({ charges, bilingual }: { charges: FormalPrintableCharges; bilingual: boolean }) {
  ...
  const pendingCount = charges.items.filter((item) => item.pendingQuote).length;
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-lg border border-slate-300 text-[10px]">
      <BusinessPendingQuoteNotice count={pendingCount} english={false} />
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-6 bg-blue-950 px-3 py-2 text-xs font-black text-white">
        <span>{pendingCount ? "已报价金额（含税）" : "折后应收（含税）"}{bilingual ? (pendingCount ? " / Quoted charges" : " / Total due") : ""}</span>
    ...
    <section className="formal-print-section mt-4">
      ...
      {(Object.keys(CATEGORY_LABELS) as Array<keyof typeof CATEGORY_LABELS>).map((kind) => {
        ...
        return <section key={kind} className="formal-print-section mt-2"><h3 className="bg-blue-50 px-2 py-1 text-[10px] font-black text-blue-900">{CATEGORY_LABELS[kind][1]} {showChinese ? <small className="font-semibold text-slate-500">{CATEGORY_LABELS[kind][0]}</small> : null}</h3><div className="grid grid-cols-[minmax(0,1.6fr)_minmax(0,1.2fr)_55px_48px_85px_80px_90px] border-b border-slate-300 px-2 py-1 text-[9px] text-slate-600"><EnglishPrimaryLabel en="Item" zh="项目名称" showChinese={showChinese} /><EnglishPrimaryLabel en="Description" zh="描述" showChinese={showChinese} /><EnglishPrimaryLabel en="Unit" zh="单位" showChinese={showChinese} /><EnglishPrimaryLabel en="Qty" zh="数量" showChinese={showChinese} /><EnglishPrimaryLabel en="Tax-incl. price" zh="含税单价" showChinese={showChinese} /><EnglishPrimaryLabel en="Item discount" zh="本项折扣" showChinese={showChinese} /><EnglishPrimaryLabel en="Tax-incl. subtotal" zh="含税小计" showChinese={showChinese} /></div>{items.map((item, index) => {
          ...
          const unitEn = item.kind === "labor" ? "JOB" : item.unitLabelEn?.trim();
          return <div key={`${kind}-${index}`} className="formal-print-row grid grid-cols-[minmax(0,1.6fr)_minmax(0,1.2fr)_55px_48px_85px_80px_90px] items-start border-b border-slate-200 px-2 py-2"><span><strong className="block">{nameEn || (showChinese ? item.nameZh : "—")}</strong>{showChinese && nameEn ? <small className="block text-[9px] text-slate-500">{item.nameZh}</small> : null}</span><span>{descriptionEn || (showChinese ? item.descriptionZh : "") || "—"}{showChinese && descriptionEn && item.descriptionZh ? <small className="block text-[9px] text-slate-500">{item.descriptionZh}</small> : null}</span><span>{unitEn || (showChinese ? item.unitLabelZh : "—")}{showChinese && item.kind !== "labor" && unitEn ? <small className="block text-[9px] text-slate-500">{item.unitLabelZh}</small> : null}</span><span>{item.quantity}</span><span className="whitespace-nowrap text-right tabular-nums">{item.pendingQuote ? "Pending quote" : formatFormalMoney(item.unitPriceMinor)}</span><span className="whitespace-nowrap text-right tabular-nums text-rose-700">{item.pendingQuote ? "—" : `−${formatFormalMoney(item.itemDiscountMinor)}`}</span><strong className="whitespace-nowrap text-right tabular-nums">{item.pendingQuote ? "—" : formatFormalMoney(item.subtotalMinor)}</strong></div>;
        })}</section>;
      })}
      <div className="mt-2 overflow-hidden rounded-lg border border-slate-300 text-[10px]"><BusinessPendingQuoteNotice count={pendingCount} english /><div className="grid grid-cols-3">{summaryRows.map(([en, zh, value, isDiscount], index) => <div key={en} className={`flex min-w-0 items-center justify-between gap-3 px-3 py-2 ${index < 3 ? "border-b" : ""} ${index % 3 !== 2 ? "border-r" : ""} border-slate-200`}><EnglishPrimaryLabel en={en} zh={zh} showChinese={showChinese} /><strong className={`shrink-0 tabular-nums ${isDiscount ? "text-rose-700" : ""}`}>{isDiscount ? "−" : ""}{formatFormalMoney(value)}</strong></div>)}</div><div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-6 bg-blue-950 px-3 py-2 text-xs font-black text-white"><EnglishPrimaryLabel en={pendingCount ? "Quoted charges (tax included)" : "Total due (tax included)"} zh={pendingCount ? "已报价金额（含税）" : "折后应收（含税）"} showChinese={showChinese} /><strong className="tabular-nums">{formatFormalMoney(totals.totalDueMinor)}</strong></div><div className="flex items-center justify-between gap-6 bg-slate-50 px-3 py-1.5 text-[9px] text-slate-600"><EnglishPrimaryLabel en="Included 15% GCT" zh="其中含 15% GCT" showChinese={showChinese} /><strong className="tabular-nums">{formatFormalMoney(totals.includedGctMinor)}</strong></div></div>
    </section>
  );
}
...
function FeeResponsibilityView({ document, snapshot }: { document: FormalBusinessOrderDocument; snapshot: FormalOfficeArchiveSnapshot | FormalCustomerCopySnapshot }) {
  return <PrintPage backHref={`/orders/business/${document.businessOrderId}`} documentNo={document.documentNo} orderNo={snapshot.businessOrder.orderNo} title="维修费用与责任确认单" titleAnnotation={snapshot.kind === "customer_copy" ? "客户联 / Customer Copy" : "办公室联 / Office Copy"}>
    <p className="my-3 font-semibold text-blue-900">非税务发票 / NOT A TAX INVOICE</p>
    <FactGrid>
      <Fact label="费用承担方 / Payer" value={snapshot.businessOrder.payerName} />
      <Fact label="联系人 / Contact" value={snapshot.businessOrder.payerContactName ?? snapshot.businessOrder.payerPhone ?? "未记录"} />
      <Fact label="TRN" value={snapshot.businessOrder.payerTrn ?? "未记录"} />
      <Fact label="车辆 / Vehicle" value={`${snapshot.businessOrder.plate} · ${snapshot.businessOrder.vehicleDescription}`} />
      <Fact label="VIN" value={snapshot.businessOrder.vin ?? "未记录"} />
      <Fact label="生成时间 / Generated" value={businessTime(document.generatedAt)} />
    </FactGrid>
    <ChargesTable charges={snapshot.charges} bilingual />
    <Notes charges={snapshot.charges} includeInternal={false} />
    <section className="formal-print-signature mt-5 rounded-lg border-2 border-slate-500 p-4">
      <p className="font-semibold">{snapshot.approval.statementZh}</p>
      <p className="mt-1 text-slate-600">{snapshot.approval.statementEn}</p>
      <div className="mt-8 grid grid-cols-2 gap-10"><span>客户签字 / Customer signature：________________________</span><span>日期 / Date：________________________</span></div>
    </section>
  </PrintPage>;
}

function MechanicView({ document, snapshot }: { document: FormalBusinessOrderDocument; snapshot: FormalMechanicWorkSnapshot }) {
  // 仅展示本单本轮绩效与工作内容；客户资料及销售收费由其他单据承载。
  return <PrintPage backHref={`/orders/business/${document.businessOrderId}`} documentNo={document.documentNo} orderNo={snapshot.businessOrder.orderNo} title="维修工联"><FactGrid><Fact label="车辆" value={`${snapshot.vehicle.plate} · ${snapshot.vehicle.description}`} /><Fact label="VIN" value={snapshot.vehicle.vin ?? "未记录"} /><Fact label="维修轮次" value={`第 ${snapshot.repairRound.roundNo} 轮维修`} /><Fact label="维修班组" value={snapshot.repairRound.teamName ?? "尚未派单"} /><Fact label="打印时间" value={businessTime(document.generatedAt)} /></FactGrid><section className="formal-print-section mt-4 rounded-lg border-2 border-blue-800 p-3"><h2 className="mb-2 text-sm font-black">本单本轮绩效 / ORDER &amp; ROUND PERFORMANCE</h2><FactGrid><Fact label="绩效值 / JMD" value={snapshot.repairRound.performanceMinor == null ? "待填写" : formatFormalMoney(snapshot.repairRound.performanceMinor)} /><Fact label="状态 / Status" value={snapshot.repairRound.performanceSource === "handoff" ? "已交单绩效" : snapshot.repairRound.performanceSource === "draft" ? "本轮预设绩效" : "尚未记录"} /><Fact label="范围 / Scope" value={`本业务单 · 第 ${snapshot.repairRound.roundNo} 轮维修`} /></FactGrid></section><section className="formal-print-section mt-4"><h2 className="border-b-2 border-slate-800 pb-1 text-sm font-black">施工项目</h2><div className="grid grid-cols-[28px_65px_minmax(150px,.9fr)_minmax(0,1.4fr)_85px] border-b border-slate-300 py-1 text-[9px] font-semibold text-slate-500"><span>完成</span><span>类别</span><span>项目名称</span><span>工作说明</span><span>数量</span></div>{snapshot.workItems.map((item, index) => <div key={`${item.kind}-${index}`} className="formal-print-row grid grid-cols-[28px_65px_minmax(150px,.9fr)_minmax(0,1.4fr)_85px] items-center border-b border-slate-200 py-2"><span className="h-4 w-4 border border-slate-700" /><span>{CATEGORY_LABELS[item.kind][0]}</span><strong>{item.nameZh}</strong><span>{item.descriptionZh || "—"}</span><span>{item.quantity} {item.kind === "labor" ? "JOB" : item.unitLabelZh}</span></div>)}</section>{snapshot.notes.length > 0 ? <section className="formal-print-section mt-4"><h2 className="border-b-2 border-slate-800 pb-1 text-sm font-black">施工备注、责任义务与提前告知</h2>{snapshot.notes.map((note, index) => <div key={`${note.kind}-${index}`} className="formal-print-row grid grid-cols-[150px_minmax(0,1fr)] border-b border-slate-200 py-2"><strong>{NOTE_LABELS[note.kind][0]}</strong><span>{note.contentZh}</span></div>)}</section> : null}<section className="formal-print-signature mt-5 rounded-lg border-2 border-slate-500 p-4"><h2 className="text-sm font-black">完成情况与回单填写</h2><p className="mt-3">实际完成内容：</p><div className="h-24 bg-[repeating-linear-gradient(to_bottom,transparent_0,transparent_23px,#cbd5e1_24px)]" /><div className="mt-5 grid grid-cols-2 gap-10"><span>实际维修人员：________________________</span><span>完成时间：________________________</span><span>维修工签字：________________________</span><span>前台代录人：________________________</span></div></section></PrintPage>;
}
...
export function FormalBusinessOrderDocumentPrintSheet({ orderId, documentId }: { orderId: number; documentId: number }) {
  ...
  if (document.snapshot.kind === "customer_copy" || document.snapshot.kind === "office_archive") {
    return <FeeResponsibilityView document={document} snapshot={document.snapshot} />;
  }
  return <MechanicView document={document} snapshot={document.snapshot} />;
}
