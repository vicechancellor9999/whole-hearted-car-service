    ...
    <div className="space-y-4">
      ...
      <section className="border-t border-line pt-4">
        ...
        {canWrite ? <div className="mt-3 grid gap-3 rounded-xl border border-line bg-surface/60 p-3 lg:grid-cols-[180px_minmax(0,1fr)]">
          <div className="space-y-2"><label className="block text-xs font-semibold">附件类别<select value={category} onChange={(event) => setCategory(event.target.value as FormalBusinessOrderAttachmentCategory)} className="mt-1 min-h-10 w-full rounded-lg border border-line bg-white px-3">{Object.entries(ATTACHMENT_CATEGORY_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="block text-xs font-semibold">说明（可选）<input value={caption} onChange={(event) => setCaption(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-line bg-white px-3" /></label></div>
          <div tabIndex={0} onDragOver={(event) => event.preventDefault()} onDrop={onDrop} onPaste={onPaste} className="grid min-h-28 place-items-center rounded-xl border border-dashed border-primary/50 bg-white p-4 text-center outline-none focus:ring-2 focus:ring-primary/20">
