...
function RepairHistoryDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--wh-overlay)] p-4" role="dialog" aria-modal="true" aria-label="Business Order 全部维修历史" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
...
export function FormalBusinessOrderDetailView({ businessOrderId }: { businessOrderId: number }) {
  ...
  const generatePrintDocument = async (kind: "customer_copy" | "office_archive" | "mechanic_work") => {
    ...
    try {
      ...
      setNotice(`已生成 ${formalDocumentKindLabel(document.kind)}：${document.documentNo}`);
      refresh();
      return document;
    } catch (caught) {
  ...
  return (
    <div data-testid="formal-business-order-detail" className="formal-business-order-page min-h-full bg-page px-3 py-3 text-ink sm:px-5">
        ...
        <header className="rounded-[22px] border border-line bg-card p-4 shadow-card">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {isFinanciallySettled ? <span className="rounded-full border border-state-success-border bg-state-success-subtle px-4 py-2 text-xs font-black text-state-success-text shadow-sm">财务已结清</span> : null}
              <span className="rounded-full bg-layer-2 px-3 py-1.5 text-xs font-bold text-accent shadow-sm">{order.voided ? "已作废" : formalBusinessOrderStatusLabel(order.status)}</span>
              <RecordDeleteButton
        ...
        {rounds && masterData ? <section id="business-order-repair-workspace" hidden={activeWorkspace !== "operations"} className="rounded-2xl border border-line bg-card p-4 shadow-card xl:col-start-2 xl:row-start-1">
          ...
          {rounds.current.status === "in_repair" ? <div className="mt-3 grid gap-3 lg:grid-cols-2">
            ...
            {advanceRoundOpen ? <form className="rounded-xl border border-primary/30 bg-primary-50 p-3 lg:col-span-2" onSubmit={(event) => { event.preventDefault(); setAdvanceRoundOpen(false); void submitRoundAction({ action: "submit_return", repairRoundVersion: rounds.current.version }, "已推进到回单待审核"); }}>
              <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-xs font-bold">推进到回单待审核</h3><p className="mt-1 text-[11px] text-ink-soft">表示维修工已完成本轮工作，等待前台现场核验；不代表正式交单或 Business Order 完结。</p></div><div className="flex gap-2"><button type="button" onClick={() => setAdvanceRoundOpen(false)} className="min-h-9 rounded-md border border-line bg-layer-2 px-3 text-xs font-bold">取消</button><button disabled={busy} className="min-h-9 rounded-lg bg-accent-solid px-4 text-xs font-bold text-accent-foreground disabled:opacity-40">确认推进</button></div></div>
            </form> : null}
          </div> : null}
            ...
            {data.capabilities.canWrite
              ...
              >
                ...
                <div className="mt-2 flex flex-wrap justify-end gap-2">
                  ...
                  >{busy ? "正在取消…" : "确认取消正式交单"}</button>
                </div>
              </form>
              : null}
            {data.capabilities.canWrite && afterSalesOpen ? <form id="formal-after-sales-form" className="mt-3 flex flex-wrap items-end gap-2 border-t border-state-success-border pt-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setAfterSalesOpen(false); void submitRoundAction({ action: "start_after_sales", businessOrderVersion: order.version, issue: String(form.get("issue")) }, "已在本 Business Order 开始下一轮售后维修"); }}><label className="min-w-[320px] flex-1 text-xs">本次售后回厂问题<input name="issue" required className="mt-1 min-h-10 w-full rounded-lg border border-line bg-layer-2 px-3 text-ink" /></label><button type="button" onClick={() => setAfterSalesOpen(false)} className="min-h-10 rounded-lg border border-line bg-layer-2 px-4 text-xs font-bold">取消</button><button disabled={busy} className="min-h-10 rounded-lg bg-amber-600 px-4 text-xs font-bold text-white">确认开始下一轮维修</button></form> : null}
          </div> : null}
      ...
      <ActionDialog open={paymentFormOpen && data.capabilities.canRecordPayment} title="登记一笔收款" description="保存后立即形成一笔不可修改的收款事实，并生成对应 Receipt。" onClose={() => setPaymentFormOpen(false)}>
        <form onSubmit={submitPayment}>
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">金额（JMD）<input name="amount" inputMode="decimal" required autoFocus className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink" /></label><label className="text-xs font-semibold">收款方式<select name="paymentMethodItemId" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink"><option value="">选择方式</option>{data.paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.labelZh}{method.labelEn ? ` / ${method.labelEn}` : ""}</option>)}</select></label></div>
          <label className="mt-3 block text-xs font-semibold">备注<input name="note" className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink" /></label>
          <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setPaymentFormOpen(false)} className="min-h-10 rounded-lg border border-line bg-layer-2 px-4 text-xs font-bold">取消</button><button disabled={busy} type="submit" className="min-h-10 rounded-lg bg-emerald-600 px-4 text-xs font-bold text-white disabled:opacity-50">{busy ? "正在登记…" : "收款并生成 Receipt"}</button></div>
        </form>
      </ActionDialog>
      <ActionDialog open={refundFormOpen && data.capabilities.canRefund} title="登记一笔退款" description="先登记退款并生成签收单；打印后由客户手写签字，签字件可随后回传。" onClose={() => setRefundFormOpen(false)}>
        <form onSubmit={submitRefund}>
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">金额（JMD）<input name="amount" inputMode="decimal" required autoFocus className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink" /></label><label className="text-xs font-semibold">退款方式<select name="paymentMethodItemId" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink"><option value="">选择方式</option>{data.paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.labelZh}{method.labelEn ? ` / ${method.labelEn}` : ""}</option>)}</select></label></div>
          <label className="mt-3 block text-xs font-semibold">退款原因<textarea name="reason" required className="mt-1 min-h-20 w-full rounded-lg border border-line bg-layer-2 p-3 text-sm text-ink" /></label>
          <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">原客户单据<select name="originalDocumentStatus" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink"><option value="returned">原单已交回</option><option value="unavailable">原单无法交回</option></select></label><label className="text-xs font-semibold">无法交回说明<input name="originalDocumentNote" className="mt-1 min-h-11 w-full rounded-lg border border-line bg-layer-2 px-3 text-sm text-ink" /></label></div>
          <p className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-xs leading-5 text-blue-800">非现金退款完成转账后，需在该笔退款记录中补传转账凭证。</p>
          <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setRefundFormOpen(false)} className="min-h-10 rounded-lg border border-line bg-layer-2 px-4 text-xs font-bold">取消</button><button disabled={busy} type="submit" className="min-h-10 rounded-lg bg-rose-600 px-4 text-xs font-bold text-white disabled:opacity-50">{busy ? "正在登记…" : "登记退款并生成签收单"}</button></div>
        </form>
      </ActionDialog>
