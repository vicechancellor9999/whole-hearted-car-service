    ...
    <div data-testid="formal-business-order-detail" className="formal-business-order-page min-h-full px-3 py-3 sm:px-5">
      ...
      <ActionDialog open={paymentFormOpen && data.capabilities.canRecordPayment} title="登记一笔收款" description="保存后立即形成一笔不可修改的收款事实，并生成对应 Receipt。" onClose={() => setPaymentFormOpen(false)}>
        <form onSubmit={submitPayment}>
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">金额（JMD）<input name="amount" inputMode="decimal" required autoFocus className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm" /></label><label className="text-xs font-semibold">收款方式<select name="paymentMethodItemId" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm"><option value="">选择方式</option>{data.paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.labelZh}{method.labelEn ? ` / ${method.labelEn}` : ""}</option>)}</select></label></div>
          <label className="mt-3 block text-xs font-semibold">备注<input name="note" className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm" /></label>
          <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setPaymentFormOpen(false)} className="min-h-10 rounded-lg border border-line bg-white px-4 text-xs font-bold">取消</button><button disabled={busy} type="submit" className="min-h-10 rounded-lg bg-emerald-600 px-4 text-xs font-bold text-white disabled:opacity-50">{busy ? "正在登记…" : "收款并生成 Receipt"}</button></div>
        </form>
      </ActionDialog>
      <ActionDialog open={refundFormOpen && data.capabilities.canRefund} title="登记一笔退款" description="先登记退款并生成签收单；打印后由客户手写签字，签字件可随后回传。" onClose={() => setRefundFormOpen(false)}>
        <form onSubmit={submitRefund}>
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">金额（JMD）<input name="amount" inputMode="decimal" required autoFocus className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm" /></label><label className="text-xs font-semibold">退款方式<select name="paymentMethodItemId" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm"><option value="">选择方式</option>{data.paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.labelZh}{method.labelEn ? ` / ${method.labelEn}` : ""}</option>)}</select></label></div>
          <label className="mt-3 block text-xs font-semibold">退款原因<textarea name="reason" required className="mt-1 min-h-20 w-full rounded-lg border border-line bg-white p-3 text-sm" /></label>
          <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold">原客户单据<select name="originalDocumentStatus" required className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm"><option value="returned">原单已交回</option><option value="unavailable">原单无法交回</option></select></label><label className="text-xs font-semibold">无法交回说明<input name="originalDocumentNote" className="mt-1 min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm" /></label></div>
          <p className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-xs leading-5 text-blue-800">非现金退款完成转账后，需在该笔退款记录中补传转账凭证。</p>
          <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setRefundFormOpen(false)} className="min-h-10 rounded-lg border border-line bg-white px-4 text-xs font-bold">取消</button><button disabled={busy} type="submit" className="min-h-10 rounded-lg bg-rose-600 px-4 text-xs font-bold text-white disabled:opacity-50">{busy ? "正在登记…" : "登记退款并生成签收单"}</button></div>
        </form>
      </ActionDialog>
