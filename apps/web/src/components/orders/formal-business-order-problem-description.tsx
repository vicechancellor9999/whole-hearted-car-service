...
export function FormalBusinessOrderProblemDescription({
  ...
  return (
    <section data-testid="business-order-problem-context" className="rounded-2xl border border-line bg-card p-4 shadow-card">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold text-ink">{english ? "Problem description" : "问题描述"}</h2>
          <p className="mt-1 text-xs text-ink-soft">{english ? "Why this Business Order exists and the scope currently being handled." : "说明为何建单，以及当前要处理的问题和范围。"}</p>
        </div>
        <button type="button" aria-expanded={historyOpen} onClick={() => setHistoryOpen((open) => !open)} className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-line bg-layer-2 px-3 text-xs font-bold text-ink">
          <Clock3 size={14} />{historyOpen ? (english ? "Hide history" : "收起历史") : (english ? "View history" : "查看历史")}
        </button>
      </div>

      <div className={`mt-3 grid gap-3 ${showRound ? "lg:grid-cols-2" : ""}`}>
        <article data-testid="business-order-current-problem" className="rounded-xl border border-accent/30 bg-accent-subtle p-4">
          <div className="flex items-center justify-between gap-3">
            <strong className="text-xs text-accent">{english ? "Current Business Order description" : "整张业务单当前问题"}</strong>
            {canWrite ? <button type="button" onClick={() => openEditor("business_order")} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-accent px-2.5 text-xs font-bold text-accent"><Pencil size={13} />{english ? "Edit" : "修改"}</button> : null}
          </div>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-ink">{orderContent ?? (english ? "No problem description was entered when this order was created." : "创建业务单时未填写问题描述。")}</p>
          {english && context.current && !context.current.contentEn ? <p className="mt-2 text-xs font-semibold text-state-warning-text">Translation required</p> : null}
        </article>

        {showRound ? <article data-testid="repair-round-current-problem" className="rounded-xl border border-state-warning-border bg-state-warning-subtle p-4">
          <div className="flex items-center justify-between gap-3">
            <strong className="text-xs text-state-warning-text">{english ? `Repair round ${context.currentRound?.roundNo} description` : `第 ${context.currentRound?.roundNo} 轮问题`}</strong>
            {canWrite ? <button type="button" onClick={() => openEditor("repair_round")} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-state-warning-border px-2.5 text-xs font-bold text-state-warning-text"><Pencil size={13} />{english ? "Edit round" : "修改本轮"}</button> : null}
          </div>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-ink">{roundContent ?? (english ? "No separate description for this repair round." : "本轮未单独填写问题描述。")}</p>
          {english && context.currentRound && !context.currentRound.contentEn ? <p className="mt-2 text-xs font-semibold text-state-warning-text">Translation required</p> : null}
        </article> : canWrite && context.currentRound ? <button type="button" onClick={() => openEditor("repair_round")} className="justify-self-start text-xs font-semibold text-primary hover:underline">{english ? "Set a separate description for this repair round" : "为本轮单独设置问题描述"}</button> : null}
      </div>
