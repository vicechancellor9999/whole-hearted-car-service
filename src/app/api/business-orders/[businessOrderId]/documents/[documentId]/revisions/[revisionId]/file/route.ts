...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (language !== "zh" && language !== "en") return NextResponse.json({ error: "打印文件语言无效" }, { status: 400 });
  ...
  try {
    ...
    const file = await runtime.documents.getRevisionFile({ documentId, revisionId, viewerAccountId: session.account.id, language });
