  ...
  return (
    <section className="records-workspace" aria-labelledby="vehicles-title">
      ...
      {canWrite ? <section className="record-editor-card"><h2>新增车辆</h2><form action={action} className="record-form record-form-eight">
        <input name="operation" type="hidden" value="create_vehicle" /><label>车牌<input name="plate" required /></label><label>VIN<input name="vin" /></label><label>品牌<input name="make" required /></label><label>车型<input name="model" required /></label><label>年份<input max="2200" min="1886" name="modelYear" type="number" /></label><label>颜色<input name="color" /></label><label>当前费用承担方<select name="ownerRef" required><OwnerOptions companies={companies} people={people} /></select></label><button type="submit">创建车辆档案</button>
      </form></section> : null}
      ...
      <section className="record-card-list">
        {vehicles.items.map((vehicle) => <article className="record-detail-card" key={vehicle.id}>
          <header><div><strong>{vehicle.plateDisplay} · {vehicle.make} {vehicle.model}</strong><small>{vehicle.vehicleNo} · {vehicle.modelYear ?? "年份未填"} · {vehicle.color ?? "颜色未填"}</small></div><span className={vehicle.hasOpenDispute ? "dispute-badge" : ""}>{vehicle.hasOpenDispute ? "客户争议中" : vehicle.isActive ? "启用" : "停用"}</span></header>
          ...
          {canWrite ? <div className="record-detail-actions">
            <details><summary>修改车辆资料</summary><form action={action} className="record-form record-inline-editor"><input name="operation" type="hidden" value="update_vehicle" /><input name="vehicleId" type="hidden" value={vehicle.id} /><input name="version" type="hidden" value={vehicle.version} /><label>车牌<input defaultValue={vehicle.plateDisplay} name="plate" required /></label><label>VIN<input defaultValue={vehicle.vin ?? ""} name="vin" /></label><label>品牌<input defaultValue={vehicle.make} name="make" required /></label><label>车型<input defaultValue={vehicle.model} name="model" required /></label><label>年份<input defaultValue={vehicle.modelYear ?? ""} name="modelYear" type="number" /></label><label>颜色<input defaultValue={vehicle.color ?? ""} name="color" /></label><label>状态<select defaultValue={String(vehicle.isActive)} name="isActive"><option value="true">启用</option><option value="false">停用</option></select></label><button type="submit">保存车辆资料</button></form></details>
            ...
            <details><summary>上传车辆附件</summary><form action={action} className="record-form record-attachment-form" encType="multipart/form-data"><input name="operation" type="hidden" value="upload_attachment" /><input name="vehicleId" type="hidden" value={vehicle.id} /><label>附件类型<select defaultValue="photo" name="kind"><option value="photo">车辆照片</option><option value="document">车辆文档</option><option value="dispute_evidence">争议凭证</option></select></label><label>选择文件<input accept="image/jpeg,image/png,image/webp,application/pdf" name="file" required type="file" /></label><label>说明<input name="caption" /></label><button type="submit">上传并归档附件</button></form></details>
          </div> : null}
        </article>)}
