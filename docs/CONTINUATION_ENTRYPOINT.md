...
- 三联生成现在会保存真实 PDF 文件及不可变版本记录。编辑区按 A4 纸张显示所有可改文字；保存时追加 R 版本，不覆盖旧版本。页面使用 PDF.js 读取同一份后端文件进行预览，下载与系统打印也读取该版本的相同字节。
- PDF 渲染器 `bo-a4-v3` 完整嵌入中英文字体并使用 10 mm 页边距。已用 Poppler 检查现有三联最新版：维修工联 1 页，客户联 2 页，办公室签字留底联 2 页，全部为 `595.28 × 841.89 pt` A4。
- 候选库已应用迁移 `0034_repair_team_sort_order`、`0035_business_order_document_revisions`。历史三联已补齐最新版 R3；原始快照和每个文件版本继续保留。
